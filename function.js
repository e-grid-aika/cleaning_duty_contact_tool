 ...
 * 掃除予定を再登録する関数
 */
function reCreateCleaningSchedule(action){
  if(action === 'confirm'){
...
function createCleaningSchedule(){
  ...
  // console.log(cleaningStartDates);
  // console.log(cleaningEndDates);
 ...
 */
function existCleaningSchedule(calendarId,cleaningStartDates,cleaningEndDates){
  
  const myCalendar = CalendarApp.getCalendarById(calendarId);
  let events = myCalendar.getEvents(cleaningStartDates,cleaningEndDates);
