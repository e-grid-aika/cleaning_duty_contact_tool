        ...
        cleaningDates.forEach((cleaningDate) => {
            ...
            if(dayOffMembers.length){
                dayOffMembers.forEach((dayOffMember)=>{
                    if((cleaningDate.toDateString() == dayOffMember[1].toDateString())&&tmpMembers.includes(dayOffMember[0])){
            ...
            while(tmpMembers.length < num){        
              ...
              var addMember = cleanedMembers.shift();
              if(!tmpMembers.includes(addMember)&&!tmpOffMembers.includes(addMember)){
